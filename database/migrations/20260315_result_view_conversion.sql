alter table if exists public.quiz_submissions
  add column if not exists result_run_token_hash text;
...
drop policy if exists "No direct access to quiz_result_views"
...
drop function if exists public.save_quiz_submission(jsonb, jsonb, jsonb, text, text);
drop function if exists public.save_quiz_submission(jsonb, jsonb, jsonb, text, text, text);
drop function if exists public.register_quiz_result_view(text, text, text);
